...
- B2B marketplace extension (`..._marketplace.sql`): `businesses` gained Saudi/compliance + trust fields (cr_number, vat_number, contact_person, mobile, national_address, cr_document_url, verification_status, response_rate, completion_score); new tables `products`, `orders`, `support_tickets`, `notifications`. Merchant flow: landing `/merchants`, onboarding wizard `/merchants/register` (skips the account step if already logged in), expanded merchant dashboard tabs (products w/ AI description, orders, customers, revenue chart, notifications, support), and an admin verification workflow (`adminSetVerification` → approve sets is_verified+active). AI features (`src/discovery/ai.ts`) use OpenRouter when `VITE_OPENROUTER_API_KEY` is set, otherwise a built-in Arabic template/FAQ fallback so they always work. Conversion components (`components/Conversion.tsx`: campaign banner, sticky CTA, exit-intent) + floating `AIAssistant` are mounted globally in `Layout`. Document upload is captured client-side (filename marker) — no Storage bucket is configured.
